...
- 修正：移除 DOCX 页面 `padding: 10px !important`。该规则曾覆盖 Word 源文件的页边距，导致带负缩进的编号或项目符号在页面左边缘被裁切；保留 `width: 100%` 以继续适应左侧预览栏。
- 修正：双栏大屏中 DOCX 页面虽已保留源页边距，但带少量左侧外溢的内容仍会贴住预览边界；将外层 `.docx-wrapper` 水平内边距调整为 20px，为负缩进、编号和项目符号提供安全留白，不覆盖文档内部页边距。
- 修正：服务器大屏的整页 CSS 宽度超过旧阈值，导致前述 DOCX 留白规则未命中；将 `.source-pane` 设为独立容器，改按其自身宽度（不超过 960px）应用 48px 水平安全留白和文档栏宽适配。规则不再受浏览器缩放、侧栏或显示器总分辨率影响。
...
## 2026-08-25 Exam preview question types
