...
## 2026-08-03 资料管理与考试管理冲突合并

- 保留当前分支的资料管理列表、筛选和操作弹窗；保留“资料管理”菜单名称及其三语专用文案。
- 合入考试管理页面所需的菜单、路由、三语文案、Vite 开发代理、Nginx 运行时代理和环境变量配置。
- 未运行构建、测试、服务或容器操作。

## 2026-08-03 资料细分方向使用 category_id

- 保留资料一级分类文本字段，二级“细分方向”选项改用后端确认的整数 `category_id`（13–18）。
- 资料列表新增“细分方向”列并直接显示 `category_id`；二级筛选、上传和编辑不再提交 `sub_category`。
- 未运行构建、测试、服务、数据库或容器操作；页面和接口联调由用户执行。

## 2026-08-03 资料分类列合并

- 资料列表移除独立“细分方向”列，在“所属类别”单元格内并排展示一级分类彩色标签和 `sub_category_name` 灰色标签。
- 保留筛选、上传、编辑、API 与 `category_id` 提交逻辑；未运行构建或自动化测试。

## 2026-08-04 Web 用户端学习中心

- `src/pages/webUser/study/index.vue`：将占位页改为学习资料浏览页，增加关键词、一级/细分方向筛选、卡片、分页，以及首次加载、空数据、失败和重试状态；筛选变化会重置到第一页，加载期间阻止重复请求。
- 学习页筛选区对齐管理端资料管理的现有格式：增加“资料筛选”标题，一级分类使用矩形按钮，细分方向使用浅灰胶囊区，资料名称输入框固定为 340px；移除独立搜索按钮，保留回车、清空和分类切换触发查询。
- `src/layouts/webUser/components/WebPageContainer.vue`：补充 `border-box`，避免公共容器的 100% 宽度叠加 28px 内边距后产生横向溢出；既有最大宽度和内边距不变。
- `src/services/webUser/study.service.js`：复用现有资料只读列表方法，集中映射分页参数和页面展示模型，不创建 Axios 实例或引入管理端写操作。
- `src/languages/modules/{zh,en,th}.ts`：补齐 `web.study` 与 `web.common.retry` 三语文案。
- 未修改公共 Web Layout 结构、路由、菜单、管理端页面、依赖、代理、部署或后端；详情/预览入口因契约未确认保持禁用。未运行构建、测试、服务或容器操作，验证由用户执行。

## 2026-08-04 Web 用户端学习中心搜索操作

- `src/pages/webUser/study/index.vue`：资料名称搜索行对齐管理端资料管理，左侧保留名称输入，右侧增加带图标的“搜索 / 重置”按钮；搜索、回车和清空均重置到第一页，重置会清空名称及两级分类后重新加载。
- 搜索按钮沿用列表加载状态并阻止重复请求；1024px 及以上保持单行左右分布，窄屏自动换行。三语输入提示统一为“请输入资料名称”语义。
- 未修改接口、路由、公共 Layout、管理端页面、数据模型、依赖或部署；未运行构建、测试、服务或容器操作，验证由用户执行。

## 2026-08-04 Web 用户端学习资料文案

- 学习中心筛选标题、分类标签、名称标签和输入提示统一使用“学习资料”表述，并同步英文、泰文；未修改布局、查询逻辑或接口。
- 未运行构建、测试、服务或容器操作，三语显示由用户验证。

## 2026-08-04 Web 用户端学习资料文件大小

- `src/services/webUser/study.service.js` 将资料列表现有 `size` 字段映射到学习资料展示模型；未修改接口参数或增加兼容字段。
- `src/pages/webUser/study/index.vue` 在卡片右上角显示文件大小，按 B、KB、MB、GB 保留一位小数，无效或非正数显示 `-`；标题继续省略，大小保持右对齐且不换行。
- 未修改卡片高度、分类、日期、查看入口、筛选、分页、路由、管理端、依赖或部署；未运行构建、测试、服务或容器操作，验证由用户执行。

## 2026-08-04 Web 用户端学习资料预览
