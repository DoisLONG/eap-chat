...
| 导入资料并生成 QA（生成练习） | 同上 | 同上 | 同上 | 同上 | `LicenseAdmin.vue` | Element Plus 上传表单 | `sop.api.js` `generateQa` | POST multipart | `/sop-api/v1/dataprep/generate_qa` | `/sop-api/ → http://$SOP_API_HOST/` | `SOP_API_HOST` | `118.196.142.69:6007` | 已确认；本阶段将现有上传 SOP→生成 QA 流程展示为“生成练习” |
