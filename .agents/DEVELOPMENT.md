...
- `src/pages/components/licenseAdmin/editDrawer.vue` 保留原文件入口，内部已由右侧抽屉改为与生成练习一致的 `720px` 居中 `el-dialog`；分类树继续由父页加载后传入，不重复请求接口。
- 编辑界面只显示所属类别、细分方向和可输入的练习描述；保存调用 `POST /v1/dataprep/sops/record/update`，提交 `record_id`、原 `title`、原 `position_id`、二级 `category_id` 与 `description`。
- QA 复核继续由 `src/components/exam/ReviewDialog.vue` 独立处理；编辑弹窗不请求或渲染 QA 数据。

## 练习生成与文件类型（本次更新）

- 生成练习弹窗只保留所属类别、细分方向、练习描述和选择文件；上传类型、解析模式、公司、部门、岗位、发布时间、结束时间及其校验、局部状态与组织查询均已移除。
- 文件选择器固定支持 `PDF`、`DOC`、`DOCX`、`XLS`、`XLSX`；前端提交文件、二级分类和可选 `description`，Dataprep 根据真实后缀决定解析器。
