...
- 视频复用现有受鉴权 Blob 加载而非直接拼接文件或对象存储 URL，避免绕过当前 token 处理。首期不保存题目时间戳，因此不承诺视频定位。

## 2026-08-26 复核页 DOCX 自适应整页预览

- 复核页 DOCX 预览改为测量原始页面宽度后整体等比缩小，以适配左侧预览区；不再要求用户横向滚动查看。
- 缩放在 `VueOfficeDocx` 的 `rendered` 回调和预览区宽度变化时执行，并锁定渲染器写入的原始页面宽度和页边距，避免窄屏媒体样式重新排版文字、表格或图片。
- 小屏下字体会随页面等比缩小；这是保留 Word 原始版式并完整显示整页的取舍。接口、文件和部署不变。

## 2026-08-26 复核页 DOCX 预览保持原始页宽

- `practiceReview` 的 DOCX 预览不再在窄容器中强制 `section.docx` 为 `width: 100%`。该规则会改变 Word 的表格和段落换行，并裁剪按原页面坐标布局的图片。
- 预览保留 DOCX 渲染器的原始页面宽度，由现有滚动容器提供横向查看；不新增前端依赖、不改文件接口或部署配置。
- 若后续需要在任意宽度下完全等比适配且保留复杂浮动图片版式，应由后端提供 DOCX 转 PDF 的预览文件，前端使用既有 PDF 预览路径。
...
- 宽度充足时保持现有原文件与题目复核双栏；不足时改为上下单栏。DOCX 适配以左侧原文件栏自身宽度为准，而非整页宽度：栏宽不足 960px 时文档页面占满栏宽、保留源文件页边距，并在外层预留水平安全留白以容纳负缩进，优先保证内容完整可读，不变更预览接口或资料格式。
