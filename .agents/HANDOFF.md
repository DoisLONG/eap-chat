...
- `src/pages/Login.vue` 已仅替换登录卡片底部品牌文案和样式：版本固定为 `v2.0.0`，年份仍由浏览器自动计算。未变更登录逻辑、接口、路由或部署配置；请用户在目标浏览器确认显示效果。

## 2026-08-28 练习复核页视频预览

- 复核页仅在 `preview_type=video` 时创建视频 Blob URL 并渲染原生播放器；页面离开或切换练习时沿用 `clearSourceUrl()` 释放 URL。视频没有题目时间戳，定位操作只显示提示。
