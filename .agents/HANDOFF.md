...
- `src/pages/practiceReview/index.vue` 已使用 CSS 容器查询，以页面实际可用宽度决定布局：内容宽度大于 1180px 保持原文件/题目复核双栏，宽度不足时自动上下排列；双栏状态中，只要左侧预览可能小于 DOCX 纸张宽度，DOCX 即填满左栏，同时保留源文件页边距，避免标题和正文被裁切。
