  ...
  <div class="exam-management">
    ...
    <el-card class="list-card">
      ...
      <el-table ref="tableRef" class="exam-table" border v-loading="loading" :data="rows" row-key="id" :empty-text="t('SopPicker.noData')" @selection-change="selectedRows = $event">
        ...
        <el-table-column :label="t('examForm.examTime')" min-width="280" align="center" header-align="center"><template #default="{ row }"><el-tooltip :content="examTimeText(row)" placement="top"><div class="exam-time-cell">{{ examTimeText(row) }}</div></el-tooltip></template></el-table-column>
...
function examTimeText(row) { const time = value => value ? formatDateTime(value, "YYYY-MM-DD HH:mm") : t("examForm.unset"); return row.start_time || row.end_time ? `${time(row.start_time)} ～ ${time(row.end_time)}` : "--"; }
