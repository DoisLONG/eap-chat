      ...
      <el-form v-loading="detailLoading" ref="formRef" :model="form" :disabled="isReadonly || detailLoading" label-position="top" class="exam-form">
        ...
        <template v-else>
          <section class="section-card">
            ...
            <div class="publish-grid"><el-form-item :label="t('exam.publishStatus')" required><el-radio-group v-model="form.publish.status"><el-radio value="draft">{{ t('examForm.saveDraft') }}</el-radio><el-radio value="publish">{{ t('exam.publishNow') }}</el-radio></el-radio-group></el-form-item><el-form-item :label="t('exam.audience')" required><el-input :model-value="t('exam.allUsers')" disabled /></el-form-item><el-form-item :label="t('exam.startTime')"><el-date-picker v-model="form.publish.startAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" /></el-form-item><el-form-item :label="t('exam.endTime')"><el-date-picker v-model="form.publish.endAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" /></el-form-item></div>
...
function validatePublish() {
  ...
  if (Boolean(startAt) !== Boolean(endAt)) throw new Error(t("examForm.timeRangeRequired"));
  if (startAt && endAt && new Date(endAt) <= new Date(startAt)) throw new Error(t("exam.invalidTime"));
  if (publishStatus === "publish" && endAt && new Date(endAt) <= new Date()) throw new Error(t("exam.invalidTime"));
}
function basePayload() { return { name: form.name, version: form.version || null, description: form.description || null, exam_type: form.type === "mixed" ? "mixed" : "normal", category_id: form.sources[0]?.category_id || null, duration: Number(form.duration), pass_score: Number(form.passScore), participant_scope: "all", random_paper: form.rules.randomPaper, randomOptions: form.rules.randomOptions, showAnswer: form.rules.showAnswer, allowRetake: form.rules.allowRetake, max_attempts: Number(form.rules.maxAttempts || 1), start_time: form.publish.startAt || null, end_time: form.publish.endAt || null }; }
