<template>
  <el-dialog v-model="visible" :title="form.id ? t('exam.edit') : t('exam.create')" width="1060px" class="exam-form-dialog" align-center :close-on-click-modal="false" :before-close="close">
      ...
      <el-form ref="formRef" :model="form" :disabled="isReadonly" label-position="top" class="exam-form">
        ...
        <template v-else>
          <section class="section-card">
            ...
            <div class="publish-grid"><el-form-item :label="t('exam.publishStatus')" required><el-radio-group v-model="form.publish.status"><el-radio value="draft">{{ t('examForm.saveDraft') }}</el-radio><el-radio value="publish">{{ t('exam.publishNow') }}</el-radio></el-radio-group></el-form-item><el-form-item :label="t('exam.audience')" required><el-input :model-value="t('exam.allUsers')" disabled /></el-form-item><template v-if="form.publish.status === 'publish'"><el-form-item :label="t('exam.startTime')"><el-date-picker v-model="form.publish.startAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" /></el-form-item><el-form-item :label="t('exam.endTime')"><el-date-picker v-model="form.publish.endAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" /></el-form-item></template></div>
            <el-alert :title="t('examForm.allAudienceOnly')" type="info" :closable="false" show-icon />
          </section>
        </template>
      </el-form>
    </div>
    <template #footer><div class="dialog-footer"><el-button v-if="step > 1" @click="step--">{{ t('exam.previous') }}</el-button><el-button @click="close">{{ t('common.cancel') }}</el-button><el-button v-if="!isReadonly" type="primary" :loading="saving" @click="next">{{ actionLabel }}</el-button></div></template>
  </el-dialog>
</template>
...
const formRef = ref(), step = ref(1), completedStep = ref(1), saving = ref(false), status = ref("draft");
...
const actionLabel = computed(() => {
  ...
  if (form.publish.status === "publish") return t("examForm.publishExam");
...
watch(() => props.modelValue, value => { if (!value) return; reset(props.exam); if (props.exam?.id) loadDetail(props.exam.id); else loadCategories(); }, { immediate: true });
...
function goToStep(target) { if (target <= completedStep.value) step.value = target; }
...
function validatePublish() {
  const { status: publishStatus, startAt, endAt, audience } = form.publish;
  ...
  if (publishStatus !== "draft" && startAt && endAt && new Date(endAt) <= new Date(startAt)) throw new Error(t("exam.invalidTime"));
  if (publishStatus === "publish" && endAt && new Date(endAt) <= new Date()) throw new Error(t("exam.invalidTime"));
}
function basePayload() { return { name: form.name, version: form.version || null, description: form.description || null, exam_type: form.type === "mixed" ? "mixed" : "normal", category_id: form.sources[0]?.category_id || null, duration: Number(form.duration), pass_score: Number(form.passScore), participant_scope: "all", random_paper: form.rules.randomPaper, randomOptions: form.rules.randomOptions, showAnswer: form.rules.showAnswer, allowRetake: form.rules.allowRetake, max_attempts: Number(form.rules.maxAttempts || 1), startAt: form.publish.startAt || null, endAt: form.publish.endAt || null }; }
...
async function next() {
  ...
  try {
    ...
    if (form.publish.status === "publish") { await ElMessageBox.confirm(t("examForm.publishConfirm"), t("header.tip"), { type: "warning" }); hydrate(await publishExam(form.id)); }
