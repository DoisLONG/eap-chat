...
export const useTable = (
  ...
  /**
   * @description 更新自定义查询参数
   * @return void
   * */
  const updatedAlignTotalParam = (searchParam) => {
    state.totalParam = {};
    // 处理查询参数，可以给查询参数加自定义前缀操作
    let nowSearchParam: Table.StateProps["searchParam"] = {};
    // 防止手动清空输入框携带参数（这里可以自定义查询参数前缀）
  ...
  /**
   * @description 自定义表格数据查询
   * @return void
   * */
  const handleAlignsearch = (searchParam) => {
    state.pageable.pageNum = 1;
    updatedAlignTotalParam(searchParam);
    getTableList();
  };
  /**
   * @description 表格数据重置
